		...
		function replyList(){
			$.ajax({
				...
				success: function(result){
					console.log(result);
					var output = "<table class='reply-list' id='reply-list'>";
					for(var i in result){
						output += "<tr>";
						output += "<td class='reply-list__userid'>"+result[i].user_id+"</td>";
						output += "<td class='reply-list__reply'>"+
												"<p><span>"+changeDate(result[i].regdate)+"</span><br>";
						output += result[i].reply_content+"</p></td>";
						output += "<td class='reply-list__btns'>"+
		                        ...
		                        "<div class='reply-report__modal-content'>"+
		                          ...
		                          "<form action='report-reply' method='post'>"+
		                            "<table><colgroup><col width='120px'><col width='350px'></colgroup>"+
		                              "<tr><th>제목</th>"+
		                                "<td><input type='text' name='report-title' placeholder='제목을 작성해주세요.'></td></tr>"+
		                              "<tr><th>신고자</th><td>user_id</td></tr>"+
		                              "<tr><th>신고 대상</th><td>user_id2</td></tr>"+
		                              "<tr><th>신고 댓글 내용</th><td>신고 대상이 작성한 댓글</td></tr>"+
		                              "<tr><th colspan='2'>신고 내용</th></tr>"+
		                              "<tr><td colspan='2'><textarea path='report' name='report' placeholder='신고 내용을 작성해 주세요'></textarea></td></tr>"+
		                              "<tr><td colspan='2' class='report-submit-btn'>"+
		                                  "<input type='submit' title='관리자에게 신고내용 전송' name='report-submit-btn' value='&#xf1d8'></td></tr></table></form>"+
		                        "</div></div>";
