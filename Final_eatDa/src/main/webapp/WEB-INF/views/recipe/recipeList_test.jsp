	...
	<div class="bottom">
		...
		<div class="paging">
			<c:if test="${rpdto.prev }" >
				<a href="recipeListTest.do?pageNum=${rpdto.startPage - 1 }&amount=${rpdto.amount }">이전</a>
			</c:if>
			
			<c:forEach var="num" begin="${rpdto.startPage }" end="${rpdto.endPage }" >
				<p class="${rpdto.pageNum eq num ? 'active' : '' }">
					<a href="recipeListTest.do?pageNum=${num }&amount=${rpdto.amount}">${num }</a>&nbsp;
				</p>
			</c:forEach>
			
			<c:if test="${rpdto.next }" >
				<a href="recipeListTest.do?pageNum=${rpdto.endPage + 1 }&amount=${rpdto.amount }">다음</a>
			</c:if>
		</div>
	</div>
